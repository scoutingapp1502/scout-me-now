...
export default function FollowInviteSection({ userId, onBack }: FollowInviteSectionProps) {
  ...
  const shareOptions: {
    ...
    },
    {
      ...
      icon: <Upload className="h-5 w-5 text-gray-900" />,
      action: () => { if (navigator.share) navigator.share({ text: inviteMessage }); },
    },
    {
      labelRo: "Copiază linkul de invitație", labelEn: "Copy invitation link",
      icon: copied ? <Check className="h-5 w-5 text-orange-500" /> : <Link className="h-5 w-5 text-gray-900" />,
      action: handleCopyLink,
    },
    {
      labelRo: "Mesaje", labelEn: "Messages",
      icon: <MessageCircle className="h-5 w-5 text-gray-900" />,
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* ── Followers section ── */}
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 pt-5 pb-2 font-body">
        ...
        <div className="mx-5 mb-3 rounded-xl border border-gray-200 bg-white px-4 py-3 flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-body mb-0.5">
              {lang === "ro" ? "Codul tău de invitație" : "Your invitation code"}
            </p>
            <p className="text-lg font-heading font-bold text-orange-500 tracking-widest">{config.invitationCode}</p>
          </div>
          <button onClick={handleCopyLink} className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
            {copied ? <Check className="h-5 w-5 text-orange-500" /> : <Copy className="h-5 w-5 text-gray-500" />}
          </button>
        </div>
