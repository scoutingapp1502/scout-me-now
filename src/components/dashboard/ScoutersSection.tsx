...
const ScoutersSection = ({ onNavigateToChat }: { onNavigateToChat?: (userId: string) => void }) => {
  ...
  useEffect(() => {
    const fetchScouts = async () => {
      ...
      // Fetch agent + club_rep user_ids to exclude them (only pure scouts here)
      const { data: excludedRoles } = await supabase
        .from("user_roles")
        .select("user_id, role")
        .in("role", ["agent", "club_rep"] as any);

      const agentUserIds = new Set((excludedRoles || []).map((r) => r.user_id));
