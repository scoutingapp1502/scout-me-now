  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5">
        <p className="text-xs text-gray-500 font-body pt-5 pb-4 leading-relaxed">
        ...
        <div className="divide-y divide-gray-200/60">
          {options.map(o => (
            <button key={o.value} onClick={() => onSelect(o.value)} className="w-full flex items-center justify-between py-3.5 hover:bg-gray-100/20 transition-colors text-left">
              <div className="flex flex-col">
                <span className="text-sm font-body text-gray-900">{lang === "ro" ? o.labelRo : o.labelEn}</span>
                {o.count != null && (
                  <span className="text-xs text-gray-500 font-body mt-0.5">
                    {o.count} {lang === "ro" ? "persoane" : "people"}
                  </span>
                )}
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ml-3 ${value === o.value ? "border-gray-900" : "border-gray-300/40"}`}>
                {value === o.value && <div className="w-2.5 h-2.5 rounded-full bg-gray-900" />}
              </div>
            </button>
          ))}
        </div>
...
function RadioRow<T extends string>({ value, current, labelRo, labelEn, lang, onSelect }: { value: T; current: T; labelRo: string; labelEn: string; lang: string; onSelect: (v: T) => void }) {
  return (
    <button onClick={() => onSelect(value)} className="w-full flex items-center justify-between py-3.5 hover:bg-gray-100/20 transition-colors text-left">
...
function VisibilityPage<T extends string>({
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="relative flex items-center px-4 py-3 border-b border-gray-200 shrink-0">
        <button onClick={onBack} className="p-1 text-gray-500 hover:text-gray-900"><ArrowLeft className="h-5 w-5" /></button>
        <h2 className="absolute left-1/2 -translate-x-1/2 font-heading text-sm tracking-wide text-gray-900 whitespace-nowrap">{lang === "ro" ? titleRo : titleEn}</h2>
        {saving && <div className="ml-auto w-4 h-4 border-2 border-orange-200 border-t-orange-500 rounded-full animate-spin" />}
      </div>
      <div className="flex-1 overflow-y-auto px-5">
        <p className="text-xs text-gray-500 font-body pt-5 pb-3 leading-relaxed">{lang === "ro" ? descRo : descEn}</p>
        <div className="divide-y divide-gray-200/60">
...
function HideUnwantedPage({ lang, value, onSelect, onBack, saving }: {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5">
        {/* Intro */}
        <p className="text-sm text-gray-500 font-body pt-5 pb-5 leading-relaxed border-b border-gray-200">
          {lang === "ro"
        ...
        {/* Section header */}
        <p className="text-sm font-body text-gray-900 pt-5 pb-3">
        ...
        <div className="space-y-1">
          {options.map(o => (
            <button key={o.value} onClick={() => onSelect(o.value)} className="w-full flex items-start justify-between py-3 hover:bg-gray-100/20 transition-colors text-left gap-3">
              <div className="flex flex-col flex-1">
                <span className="text-sm font-semibold font-body text-gray-900">{lang === "ro" ? o.labelRo : o.labelEn}</span>
                <span className="text-sm text-gray-500 font-body mt-0.5 leading-snug">{lang === "ro" ? o.descRo : o.descEn}</span>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${value === o.value ? "border-gray-900" : "border-gray-300/40"}`}>
                {value === o.value && <div className="w-2.5 h-2.5 rounded-full bg-gray-900" />}
              </div>
            </button>
          ))}
...
function BlockCommentersPage({ userId, lang, onBack }: { userId: string; lang: string; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="relative flex items-center px-4 py-3 border-b border-gray-200 shrink-0">
        <button onClick={onBack} className="p-1 text-gray-500 hover:text-gray-900"><ArrowLeft className="h-5 w-5" /></button>
        <h2 className="absolute left-1/2 -translate-x-1/2 font-heading text-sm tracking-wide text-gray-900 whitespace-nowrap">{lang === "ro" ? "Blochează comentarii de la" : "Block comments from"}</h2>
      </div>
      ...
      <div className="px-4 py-3 border-b border-gray-200 shrink-0">
        <input
          ...
          className="w-full bg-gray-100 rounded-xl px-3 py-2.5 text-sm font-body outline-none text-gray-900 placeholder:text-gray-500"
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="border-b border-gray-200">
            {results.map(u => (
              <div key={u.user_id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-200/40 last:border-0">
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-semibold font-body text-gray-900 truncate">{`${u.first_name ?? ""} ${u.last_name ?? ""}`.trim()}</p></div>
        ...
        {query.trim() && results.length === 0 && (
          <p className="text-sm text-gray-500 font-body text-center py-8">{lang === "ro" ? "Niciun utilizator găsit." : "No users found."}</p>
        )}

        {/* Blocked list */}
        {!query.trim() && (
          loading
            ? <div className="flex justify-center py-10"><div className="w-5 h-5 border-2 border-orange-200 border-t-orange-500 rounded-full animate-spin" /></div>
            : blocked.length === 0
              ? <p className="text-sm text-gray-500 font-body text-center py-12">{lang === "ro" ? "Nicio persoană blocată din comentarii." : "No one blocked from commenting."}</p>
              : <>{blocked.map(u => (
                  <div key={u.id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-200/40">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold font-body text-gray-900 truncate">{u.name}</p></div>
                    <button onClick={() => handleUnblock(u.id, u.name)} className="px-3 py-1.5 rounded-lg bg-orange-500 text-white text-sm font-semibold">{lang === "ro" ? "Deblochează" : "Unblock"}</button>
                  </div>
                ))}</>
        )}
      </div>
...
export default function CommentsSection({ userId, onBack }: CommentsSectionProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="relative flex items-center px-4 py-3 border-b border-gray-200 shrink-0">
        <button onClick={onBack} className="p-1 text-gray-500 hover:text-gray-900"><ArrowLeft className="h-5 w-5" /></button>
        <h2 className="absolute left-1/2 -translate-x-1/2 font-heading text-sm tracking-wide text-gray-900 whitespace-nowrap">{lang === "ro" ? "Comentarii" : "Comments"}</h2>
        {saving && <div className="ml-auto w-4 h-4 border-2 border-orange-200 border-t-orange-500 rounded-full animate-spin" />}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-white border-b border-gray-200">
          <button onClick={() => { setBlockCommentersReturn(null); setSubPage("block-commenters"); }} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-100/30 transition-colors">
            <span className="text-sm font-body text-gray-900">{lang === "ro" ? "Blochează comentarii de la" : "Block comments from"}</span>
            <div className="flex items-center gap-1">
              <span className="text-sm text-gray-500 font-body">{blockedCount} {lang === "ro" ? "persoane" : "people"}</span>
              <ChevronRight className="h-4 w-4 text-gray-500" />
            </div>
          </button>
        </div>
        <p className="text-xs text-gray-500 font-body px-5 py-3 leading-relaxed border-b border-gray-200">
          {lang === "ro"
        ...
        <div className="px-5 pt-5 pb-2">
          <p className="text-sm font-bold font-body text-gray-900">{lang === "ro" ? "Cine poate comenta" : "Who can comment"}</p>
        </div>
        <div className="bg-white border-y border-gray-200 divide-y divide-gray-200">
          <button onClick={() => setSubPage("posts-visibility")} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-100/30 transition-colors">
            <span className="text-sm font-body text-gray-900">{lang === "ro" ? "Postări și reels" : "Posts and reels"}</span>
            <div className="flex items-center gap-1">
              <span className="text-sm text-gray-500 font-body">{lang === "ro" ? POSTS_LABEL_MAP[config.postsVisibility].ro : POSTS_LABEL_MAP[config.postsVisibility].en}</span>
              <ChevronRight className="h-4 w-4 text-gray-500" />
            </div>
          </button>
        </div>
        ...
        {/* Types of comments */}
        <div className="px-5 pt-5 pb-2">
          <p className="text-sm font-bold font-body text-gray-900">{lang === "ro" ? "Tipuri de comentarii" : "Types of comments"}</p>
        </div>
        <div className="bg-white border-y border-gray-200">
          <button onClick={() => setSubPage("hide-unwanted")} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-100/30 transition-colors">
            <span className="text-sm font-body text-gray-900">{lang === "ro" ? "Ascunde comentarii nedorite" : "Hide unwanted comments"}</span>
            <div className="flex items-center gap-1">
              <span className="text-sm text-gray-500 font-body">{{off:"Niciuna",some:"Unele",more:"Mai multe"}[config.hideUnwanted]}</span>
              <ChevronRight className="h-4 w-4 text-gray-500" />
            </div>
          </button>
        </div>
