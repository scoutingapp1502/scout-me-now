...
function RadioRow<T extends string>({
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-between py-3.5 hover:bg-gray-50 transition-colors text-left"
    >
      <span className="text-sm font-body text-gray-900">{lang === "ro" ? labelRo : labelEn}</span>
      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${current === value ? "border-gray-900" : "border-gray-300/40"}`}>
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5">
        <p className="text-sm text-gray-500 font-body pt-5 pb-1 leading-relaxed">
          {lang === "ro"
            ? "Alege cine te poate adăuga direct într-o conversație de grup."
            : "Choose who can add you directly to a group chat."}
        </p>

        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider pt-4 pb-1">
...
function StoryRepliesPage({
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5">
        <p className="text-xs text-gray-500 font-body pt-5 pb-3 leading-relaxed">
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        <p className="text-xs text-gray-500 font-body uppercase tracking-wider px-5 pt-5 pb-2">
        ...
        <div className="bg-white border-y border-gray-200 divide-y divide-gray-200">
          <button onClick={() => setSubPage("group-requests")} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors">
            <span className="text-sm font-body text-gray-900">{lang === "ro" ? "Solicitări de grup" : "Group requests"}</span>
            <div className="flex items-center gap-1">
              <span className="text-sm text-gray-500 font-body">{labelGroupChat()}</span>
              <ChevronRight className="h-4 w-4 text-gray-500" />
            </div>
          </button>
          <button onClick={() => setSubPage("story-replies")} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors">
            <span className="text-sm font-body text-gray-900">{lang === "ro" ? "Răspunsuri la story" : "Story replies"}</span>
            <div className="flex items-center gap-1">
              <span className="text-sm text-gray-500 font-body">{labelStory()}</span>
              <ChevronRight className="h-4 w-4 text-gray-500" />
            </div>
          </button>
        </div>
