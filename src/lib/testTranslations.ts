...
const testDescriptionTranslations: Record<string, Partial<Record<Language, string>>> = {
  ...
  endurance: {
    en: "Setup: On a basketball court. The player positions themselves at the middle of the free-throw lane (the key).\n\nExercise: On the whistle, the player moves laterally to one line of the lane, touching it with the foot, then turns and runs to the other line of the lane, touching it as well. Once both lines of the lane have been touched, the exercise ends. The total time is measured.",
    de: "Aufbau: Auf dem Basketballfeld. Der Spieler positioniert sich in der Mitte der Freiwurfzone.\n\nÜbung: Beim Pfiff bewegt sich der Spieler seitlich zu einer Linie der Zone, berührt sie mit dem Fuß, dreht sich dann um und läuft zur anderen Linie der Zone, die er ebenfalls berührt. Sobald beide Linien der Zone berührt wurden, endet die Übung. Die Gesamtzeit wird gemessen.",
    fr: "Configuration : Sur un terrain de basket-ball. Le joueur se positionne au milieu de la raquette.\n\nExercice : Au coup de sifflet, le joueur se déplace latéralement jusqu'à une ligne de la raquette, la touche du pied, puis se retourne et court vers l'autre ligne de la raquette, qu'il touche également. Une fois les deux lignes de la raquette touchées, l'exercice se termine. Le temps total est chronométré.",
    es: "Configuración: En una cancha de baloncesto. El jugador se coloca en el centro de la zona.\n\nEjercicio: Al silbato, el jugador se desplaza lateralmente hasta una línea de la zona, tocándola con el pie, y luego se da la vuelta y corre hacia la otra línea de la zona, que también toca. Una vez tocadas ambas líneas de la zona, el ejercicio termina. Se cronometra el tiempo total.",
    it: "Configurazione: Su un campo da basket. Il giocatore si posiziona al centro dell'area dei tre secondi.\n\nEsercizio: Al fischio, il giocatore si sposta lateralmente fino a una linea dell'area, toccandola con il piede, poi si gira e corre verso l'altra linea dell'area, toccando anch'essa. Una volta toccate entrambe le linee dell'area, l'esercizio termina. Viene cronometrato il tempo totale.",
  },
  acceleration: {
