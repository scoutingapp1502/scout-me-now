-- get_notifications_feed() built other_name as
-- COALESCE(p.first_name || ' ' || p.last_name, 'Unknown') everywhere. That
-- only falls back when the concatenation is NULL (i.e. no matching profile
-- row at all) — a profile row that exists but has empty-string
-- first_name/last_name (a draft/incomplete profile, '' not NULL) produces
-- '' || ' ' || '' = ' ' (a single space), which is non-null so COALESCE
-- never replaces it. The notification then renders with a blank name
-- before the message text instead of "Unknown"/the person's name, exactly
-- what was reported. Every occurrence is replaced with
-- NULLIF(TRIM(...), '') wrapped in the same COALESCE so both "no profile
-- row" and "profile row with blank name" fall back correctly.
...
AS $$
  ...
  incoming_follows AS (
    SELECT
      ...
      COALESCE(NULLIF(TRIM(p.first_name || ' ' || p.last_name), ''), 'Unknown') AS other_name,
  ...
  outgoing_follows AS (
    SELECT
      ...
      f.following_id,
      COALESCE(NULLIF(TRIM(p.first_name || ' ' || p.last_name), ''), 'Unknown'),
      p.photo_url,
  ...
  video_notifs AS (
    SELECT
      ...
      vn.created_at,
      vn.player_id,
      COALESCE(NULLIF(TRIM(p.first_name || ' ' || p.last_name), ''), 'Jucător'),
      p.photo_url, 'player'::text,
      NULL::text, NULL::text,
      NULL::text, NULL::text,
