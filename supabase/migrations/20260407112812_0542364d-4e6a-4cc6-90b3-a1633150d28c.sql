...
BEGIN
  ...
  -- Parse sports array from metadata for scouts/agents
  ...
  IF _role = 'player' THEN
    ...
    ON CONFLICT (user_id) DO NOTHING;
  ELSE
    INSERT INTO public.scout_profiles (user_id, first_name, last_name, gender, sports)
    VALUES (NEW.id, _first_name, _last_name, _gender, _sports)
    ON CONFLICT (user_id) DO NOTHING;
  END IF;
